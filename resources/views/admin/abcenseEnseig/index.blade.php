...
<div class="container-fluid">
    <div class="row">
        <div class="col-12 mx-auto"> <!-- Added mx-auto class here -->
            ...
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">abcenseEnseig</h4>
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered zero-configuration">
                            ...
                            <tbody align="center">
                                ...
                                <tr>
                                    <td class="text-center">{{$abcense->enseignant->nom .' '.$abcense->enseignant->prenom }}</td>
                                    <td class="text-center">{{$abcense->emploieEns->jour ?? '-' }}</td>
                                    <td class="text-center">{{$abcense->arrived_at}}</td>
                                    <td class="text-center">
                                        {{ $abcense->is_justified ? 'Justified' : 'Not Justified' }}
                                    </td>

                                    <td class="text-center">
                                        <div class="btn-group-custom">
                                            <a href="{{ route('abcenseEnseig.edit',$abcense->id) }}" class="editBtn" data-toggle="modal" data-target="#Editnew{{ $abcense->id }}">
                                            ...
                                            <form action="{{ route('abcenseEnseig.destroy', $abcense->id) }}" method="POST" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        <div class="modal-content">
                                            <div class="container-fluid">
                                                <!-- Form for creating new data -->
                                                    <form class="form-valide" action="{{ route('abcenseEnseig.update',$abcense->id) }}" method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                        <div class="form-group">
                                                            <label class="col-lg-4 col-form-label" for="enseignant_id">Enseignant</label>
                                                            <select name="enseignant_id" id="enseignant_id" class="form-control selected" value="{{  $abcense->enseignant_id }}">
                                                                <option value="">open select enseignant</option>
                                                                @foreach ($enseignants as $enseignant)
                                                                <option value="{{ $enseignant->id }}"{{ $abcense->enseignant_id == $enseignant->id ? 'selected' : '' }}>{{ $enseignant->nom.' '.$enseignant->prenom }}</option>
                                                                @endforeach
                                                            </select>
                                                            @error('enseignant_id')
                                                        ...
                                                        <div class="form-group">
                                                            <label class="col-lg-4 col-form-label" for="emploie_ens_id">Jour</label>
                                                            <select name="emploie_ens_id" id="emploie_ens_id" class="form-control selected" value="{{  $abcense->emploie_ens_id }}">
                                                                <option value="">select jour</option>
                                                                @foreach ($emploies as $emploie)
                                                                    <option value="{{ $emploie->id }}"{{ $abcense->emploie_ens_id == $emploie->id ? 'selected' : '' }}>{{ $emploie->jour }}</option>
                                                                @endforeach
                                                            </select>
                                                            @error('emploie_ens_id')
                                                                <div class="alert alert-danger">{{ $message }}</div>
                                                            @enderror
                                                        </div>
                                                    <div class="form-group">
...
<div class="modal fade" id="Addnew" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="container-fluid">
                <!-- Form for creating new data -->
                    <form class="form-valide" action="{{ route('abcenseEnseig.store') }}" method="POST">
                        @csrf
                        <div class="form-group">
                            <label class="col-lg-4 col-form-label" for="enseignant_id">Enseignant</label>
                        ...
                        <div class="form-group">
                            <label class="col-lg-4 col-form-label" for="emploie_ens_id">Jour</label>
                            <select name="emploie_ens_id" id="emploie_ens_id" class="form-control selected">
                                <option value="">select jour</option>
                                @foreach ($emploies as $emploie)
