                        ...
                        <table class="table table-striped table-bordered zero-configuration">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">{{$groupe->formation->nom }}</td>
                                    {{-- <td class="text-center">{{$groupe->etudiant}}</td> --}}
                                    <td class="text-center">
                                        <div class="btn-group-custom">
                                            <a href="{{ route('g.edit',$groupe->id) }}" class="editBtn" data-toggle="modal" data-target="#Editnew{{ $groupe->id }}">
                                            ...
                                            <a href="{{route('g.show',$groupe->id)}}" class="editBtn" >Show</a>

                                            <form action="{{ route('g.destroy', $groupe->id) }}" method="POST" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <div class="modal fade" id="Editnew{{ $groupe->id }}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                                    <div class="modal-dialog modal-lg">
                                        <div class="modal-content">
                                            <div class="container-fluid">
                                                <!-- Form for editing existing data -->
                                        <form action="{{ route('g.update', $groupe->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            <div class="form-group">
                                                <label class="col-lg-4 col-form-label" for="nom"> Groupe</label>
                                                <input type="text" name="nom" id="nom" value="{{ $groupe->nom }}"  class="form-control">
                                                @error('nom')
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select name="formation_id" id="formation_id"  class="form-control selected">
                                                    ...
                                                    <option value="{{ $formation->id }}"{{ $groupe->formation_id == $formation->id ? 'selected' : ' ' }}>{{ $formation->nom }}</option>
                                                    @endforeach
                                                </select>
                                                @error('formation_id')
        ...
        <div class="modal-content">
            <div class="container-fluid">
                <!-- Form for creating new data -->
                    <form class="form-valide" action="{{ route('g.store') }}" method="POST">
                        @csrf
                        <div class="form-group">
                            <label class="col-lg-4 col-form-label" for="nom"> Groupe</label>
