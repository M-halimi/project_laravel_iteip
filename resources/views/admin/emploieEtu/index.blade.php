        ...
        <div class="row">
            <div class="col-12 mx-auto"> <!-- Added mx-auto class here -->
                <button class="cssbuttons-io-button" type="button" data-toggle="modal" data-target="#Addnew">
                    <span>Add New EmploieEtud</span>
                </button>
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">EmploieEtud</h4>
                                    ...
                                    @foreach ($emploies as $emploie)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                {{ $emploie->etudiant->nom . ' ' . $emploie->etudiant->prenom }}</td>
                                            <td class="text-center">{{ $emploie->groupe->nom }}</td>
                                            <td class="text-center">{{ $emploie->module->nom }}</td>
                                            <td class="text-center">
                                                <div class="btn-group-custom">
                                                    <a href="{{ route('emploiEtuds.edit',$emploie->id) }}" class="editBtn" data-toggle="modal" data-target="#Editnew{{ $emploie->id }}">
                                                    ...
                                                    <form action="{{ route('emploiEtuds.destroy', $emploie->id) }}" method="POST" style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
                                                ...
                                                <div class="modal-content">
                                                    <div class="container-fluid">
                                                        <!-- Form for creating new data -->
                                                            <form class="form-valide" action="{{ route('emploiEtuds.update',$emploie->id) }}" method="POST">
                                                                @csrf
                                                                @method('PUT')
                                                                ...
                                                                <div class="form-group">
                                                                    <label class="col-lg-4 col-form-label" for="etudiant_id">Formation</label>
                                                                    <select name="etudiant_id" id="etudiant_id"  class="form-control selected" value="{{ $emploie->etudiant_id }}">
                                                                        <option value="selected">Open this select Formation</option>
                                                                        @foreach($etudiants as $etudiant)
                                                                        <option value="{{ $etudiant->id }}"{{ $emploie->etudiant_id == $etudiant->id ? 'selected' : ' ' }}>{{ $etudiant->nom.' '.$etudiant->prenom}}</option>
                                                                        @endforeach
                                                                    </select>
                                                                    @error('enseignant_id')
    ...
    <div class="modal fade" id="Addnew" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel"aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="container-fluid">
                    <!-- Form for creating new data -->
                    <form class="form-valide" action="{{ route('emploiEtuds.store') }}" method="POST">
                        @csrf
                        <div class="form-group">
                            <label class="col-lg-4 col-form-label" for="jour">Jour</label>
                        ...
                        <div class="form-group">
                            ...
                            @error('etudiant_id')
                                <div class="alert alert-danger mt-5">gggggg</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            <label class="col-lg-4 col-form-label" for="groupe_id">Groupe</label>
